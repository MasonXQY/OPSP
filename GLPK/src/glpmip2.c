...
static void log_progress(MIPTREE *tree)
{     
	if(file_log==NULL)
	{
		file_log=fopen("log_progress.txt","w+");
	}
	if(file_log!=NULL)
	{
		fprintf(file_log,"\n%lf,%lf",utime()-tree->tm_beg,tree->best);
		fflush(file_log);
	}
}
#endif
...
#ifdef REM_ZHOUANSHI
	  lpx_set_real_parm(tree->lp,LPX_K_TMLIM,tree->tm_lim/100);
#endif
