...
{     /* process data cards in COLUMNS section */
      ...
      map = ucalloc(1+m, sizeof(int));
      for (i = 1; i <= m; i++) map[i] = 0;
      ind = ucalloc(1+m, sizeof(int));
      aij = ucalloc(1+m, sizeof(double));
loop: /* read and split next data card */
      if (read_card(dsa)) goto fail;
      if (dsa->card[0] != ' ') goto done;
      if (split_card(dsa)) goto fail;
...
skip: /* scan row name and coefficient in fields 3 and 4 */
      ...
      ufree(map), ufree(ind), ufree(aij);
      return 0;
fail: ufree(map), ufree(ind), ufree(aij);
      return 1;
}
...
{     FILE *fp;
      ...
      /*output the objective row whether there is row with no bnds zhangning20120517*/
	  /*if (make_obj == 2)
      {  for (i = 1; i <= nrows; i++)
         {  int typx;
